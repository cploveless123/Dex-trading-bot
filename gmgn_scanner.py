...
def scan_token(token_data, reason_if_fail=None):
    ...
    # Age check (fallback: skip if age=0 from GMGN - trust it's new enough)
    if age_sec < MIN_AGE_SECONDS:
        if age_sec == 0:
            pass  # Trust GMGN tokens are new
        else:
            return None, f"age {age_sec}s < {MIN_AGE_SECONDS}s"
    ...
    # 5m volume: GMGN trending may not return it - only check if provided
    vol_5m = float(token_data.get('volume5m', 0) or 0)
    if vol_5m > 0 and vol_5m < MIN_5MIN_VOLUME:
        return None, f"5m vol ${vol_5m:,.0f} < ${MIN_5MIN_VOLUME:,}"

    # Vol/Mcap ratio
    if vol_mcap_ratio < VOL_MCAP_RATIO_MIN: return None, f"vol/mcap {vol_mcap_ratio:.2f} < {VOL_MCAP_RATIO_MIN}"

    # Buy/Sell ratio
    # Buy/Sell ratio: only enforce for non-pump.fun tokens
    if launchpad == 'pump':
        pass  # pump.fun has no BS data, trust it
    elif age_sec < 900:
        if bs_ratio > 0 and bs_ratio < BS_RATIO_NEW: return None, f"BS {bs_ratio:.2f} < {BS_RATIO_NEW} (<15min)"
    else:
        if bs_ratio > 0 and bs_ratio < BS_RATIO_OLD: return None, f"BS {bs_ratio:.2f} < {BS_RATIO_OLD} (>15min)"
...
def add_to_cooldown(addr, token_data, result, entry_chg5):
    ...
    COOLDOWN_WATCH[addr] = {
        ...
        'lowest_chg5': float(chg5),  # Track lowest chg5 for recovery
...
def scan_cycle():
    ...
    for addr, data in list(COOLDOWN_WATCH.items()):
        ...
        # === CHG1 < -5% PATH: 15s rechecks until mcap > +5% from lowest ===
        if state == STATE_CHG1_RECHECK:
...
def main():
    ...
    print(f"  CHG1 recovery: chg1<-5% → recheck every 15s until mcap>+5%from low → verify 15s → BUY")
    print(f"  Entry: Young<15min+older+momentum→45s | base→30s verify chg1>+3%from last")
